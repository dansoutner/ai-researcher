"""MCP Integration Example for Agent V3 (Claude).

This example shows how to integrate MCP tools with Agent V3.
"""
...
async def get_agent_v3_tools_with_mcp(
    include_pexlib: bool = False,
    include_arxiv: bool = False,
    verbose: bool = False,
) -> List[BaseTool]:
    """Get Agent V3 tools combined with MCP tools.
    ...
    Returns:
        List of all tools for Agent V3
    """
    from ai_researcher.mcp_integration import get_mcp_tools_by_name

    # Get base agent v3 tools
    # TODO: Import agent v3 base tools here when available
    # from ai_researcher.agent_v3_claude.tools import get_default_tools
    ...
    if server_names:
        try:
            ...
            if verbose:
                print(f"✓ Added {len(mcp_tools)} MCP tools to Agent V3")
...
async def demo():
    """Demo Agent V3 with MCP tools."""
    print("=== Agent V3 + MCP Integration Demo ===\n")
